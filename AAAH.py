...
def main():
    """Main function. Presents the AAAH CLI."""
    ...
    while True:
        ...
        if (window_x > 50):
            screen.addstr(
                0, (window_x-len(title_string))/2, title_string, curses.A_BOLD)
        ...
        if command == curses.KEY_DOWN and \
                pad_row <= appmtCount - visibleRows - 2:
            pad_row += 1
            ...
            #   user_input = ''
                ...
                if not appointmentExistsSQL(uid):
                    ...
                    sendCancellation(
                        ...
                        row[9])  # end time 12H
                    feedback = 'Cancellation email sent for appointment ' + \
                        user_input + ' [' + row[1] + ']'
                    user_input = ''
                    # sleep for 1 second gives handler time to remove appt.
                    sleep(1)
                    appmtCount = appointmentCountSQL()
                    appmtList = getAllAppointmentsSQL()
                    if appmtCount > 1:
